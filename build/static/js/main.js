...
$('.sort-arrow').click(function () {
  let sort_arrow_parent = $(this).parent().attr('class')

  for(let i = 0; i < $(`.${sort_arrow_parent}`).find('.sort-arrow').length; i++) {
    if(this === $(`.${sort_arrow_parent}`).children('.sort-arrow')[i]) {
      $($(`.${sort_arrow_parent}`).children('.sort-arrow')[i]).toggleClass('active-sort-arrow')
    }else{
      $($(`.${sort_arrow_parent}`).children('.sort-arrow')[i]).removeClass('active-sort-arrow')
    }
  }
})
...
$(document).ready(function () {
  ...
  var chek_has_parent = wrap => {
    if (!$(`.${wrap}`).parent().hasClass('.partial-checkbox')) {
      return ($(`.${wrap}`).children('input[type="checkbox"]').filter(':checked').length);
    }
  };

  $('input[type=\'checkbox\']').change(function () {
    let check = chek_has_parent('checkbox-custom');
    ...
    if (check === 0) {
      ...
      $('.categories-edit').addClass('disabled');
      $('.categories-edit').prop('disabled', true);
      $('.edit-document').addClass('disabled');
      $('.edit-document').prop('disabled', true);
      $('.role_edit_tool').addClass('disabled');
      $('.role_edit_tool').prop('disabled', true);

      $('.remove').addClass('disabled');
      $('.remove').prop('disabled', true);
      $('.categories-remove').addClass('disabled');
      $('.categories-remove').prop('disabled', true);
      $('.remove-document').addClass('disabled');
      $('.remove-document').prop('disabled', true);
    } else if (check > 0) {
...
$(document).ready(function () {
  let array = [
    ...
    'announcements', 'filter', 'role', 'type'
  ];

  $('.options-users').find('.sort-svg').click(function () {
    let data = $(this).closest('.options-users').attr('id');
...
$(document).ready(function () {
  ...
  $('.announcement-line, .modal-toggle-announcement').click(function () {
    ...
    if(photos_length > 5){
      ...
      toggle_class.find('.announcement-all-images').append(hidden_photos)
    }
  });
  ...
  $('.user-marketplace, .modal-toggle-user-list').click(function () {
    ...
    if(photos_length > 5) {
      ...
      user_list_modal.find('.user-list-all-images').append(hidden_photos)
    }
  });
  $('.tool-seller-btn').click(function () {
...
$(document).ready(function () {
  ...
  let all_photos_length = 1;
  let all_photos_boolean = true;

  main_parent.on('change', 'input[type=\'file\']', function () {
    $(this).parent().remove();
    let photos_length = this.files.length - 1;
    for (let i = 0; i <= this.files.length; i++) {
      ...
      fileReader.onload = function (e) {
        ...
        let text = `<li class="photos-all">
                            ...
                            <img src="${target.result}" alt="" id="img" style="visibility: visible">
                        </li>`;
        main_parent.append(text);

        if (i === photos_length && all_photos_length <= 20) {
          setTimeout(() => {
            main_parent.append(`<li class="photos-all">
                            ...
                            <input type="file" multiple class="file-all" accept="image/*" />
                        </li>`);
          }, 500);
          all_photos_boolean = false;
        }
      };
      if (all_photos_length === 21) {
        all_photos_boolean = true;
        break;
      }
  ...
  main_parent.on('click', 'svg', function () {
    $(this).parent().remove();
    all_photos_length--;
    if (all_photos_length <= 20 && all_photos_boolean) {
      setTimeout(() => {
        main_parent.append(`<li class="photos-all">
                            <svg width="10"
                                 height="10"
...
$(document).ready(function () {
  $('.categories-padding').children('button').click(function (e) {
    e.preventDefault();
    $(this).closest('.categories-line-wrap').children('.subcategories-drop').slideToggle(400);
    $(this).closest('.categories-line-wrap').toggleClass('categories-active');
